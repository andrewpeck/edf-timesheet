...
:VISIBILITY: children
:END:
** Cumulative Project Distribution
#+begin_src emacs-lisp :exports both :results output
(let ((org-table-data ()))

  (save-excursion
    (dolist (table (append
                    (mapcar (lambda (x) (format "2021-%02d" x)) (number-sequence 1 12)) ;; 2021
                    (mapcar (lambda (x) (format "2022-%02d" x)) (number-sequence 1  2)) ;; 2022
                    ))
      (goto-char (point-min))
  ...
  (setq org-table-data
        (cl-remove-if
         (lambda (a)
           (member (nth 3 a) '("VACATION" "HOLIDAY" "SICK" "ADMIN" "VAC" "DEVEL")))
         org-table-data))
...
| 2022 January   | https://docs.google.com/spreadsheets/d/19l8IQrF-k94dgKQt96BDtDHHMP8ZtamEpXO8dLmdn7w/edit#gid=1295694596 |
| 2022 February  |                                                                                                         |
|----------------+---------------------------------------------------------------------------------------------------------|
...
| CMS-EMU-UPGRADE-GE21 | 2022 |  2 |  5.0 | $ 445.0 |
...
| CMS-EMU-UPGRADE-ME0  | 2022 |  2 |  2.5 | $ 222.5 |
...
(defun filter-work-chart (data) (cl-remove-if (lambda (a) (string= "" (car a)))
  (cdr (mapcar (lambda (x) (list (upcase (nth 3 x )) (if (stringp (nth 6 x))
  ...
  (let ((totals ())
    ...
    (dolist (item totals)
      (let* ((key (car item))
        ...
        (when (not (equal key "--"))
          (princ (format "%5.1f   %2d%% %20s %s\n" count percent key
                         (make-string (round count-normal) ?+))))))
    (princ (format "%s\n" (make-string 32 ?-)))
    (princ (format "%5.1f" sum))))

(defun plot-monthly-work-chart (data)
  (plot-chart
  (filter-work-chart data) "Hours" "Project" 50 t) )

;; (defun plot-monthly-work-chart (data &optional normalize)
;;   (let ((totals ())
;;         (ht (make-hash-table :test 'equal)))
...
;;       (let ((day      (nth 1 row))
;;             (times    (nth 2 row))
;;             (project  (upcase (nth 3 row)))
;;             (task     (nth 4 row))
;;             (wkday    (nth 5 row))
;;             (hours    (nth 6 row)))
;;         (let ((hours (if (stringp hours) (string-to-number hours) hours)))
;;           (when (and (> hours 0) (numberp hours))
;;             (puthash project (+ hours (gethash project ht 0)) ht)))))
...
;;       (let* ((key (car item))
;;              (count (cadr item))
;;              (count-normal (if normalize (* 75 (/ count max-val)) count))
;;              (percent (* 100 (/ count sum))))
;;         (when (not (equal key "--"))
;;           (princ (format "%5.1f   %2d%% %20s %s\n" count percent key
;;                          (make-string (round count-normal) ?+)))))))
...
| # | 12 |      1:30-2 | ME0      | Repo configuration         | TUE |   0.5 |
...
| # | 14 |        12-1 | ME0      | Repository                 | THU |   1.0 |
...
| # | 25 |        12-1 | ME0      | Meeting (Evaldas)          | MON |   1.0 |
...
| # | 28 |    12-12:30 | ME0      | Firmware                   | THU |   0.5 |
...
| # | 29 |         2-3 | ME0      | Meeting (UCLA)             | FRI |   1.0 |
...
(plot-monthly-work-chart data)
;; (plot-chart (filter-work-chart data) "Hours" "Project" t t)
#+end_src
...
| # | 7 |       12-1 | ME0     | Chloe                | MON |   1.0 |
| # | 7 |        2-6 | ETL     | Mockup               | MON |   4.0 |
| # |   |            |         |                      |     |       |
| # | 9 |      10-11 | ME0     | Meeting              | WED |   1.0 |
| # | 9 |        4-6 | CSC     | ODMB Integration     | WED |   2.0 |
| # | 9 |       11-4 | ETL     | Mockup               | WED |   5.0 |
| # |   |            |         |                      |     |       |
|---+---+------------+---------+----------------------+-----+-------|
